...
\begin{figure}[ht]
	\centering
	\includegraphics[width=.45\textwidth]{img/succ_rej.png}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=.55\textwidth]{img/seq_halv.png}
...
\setstretch{1.1}
	...
	\Repeat{$b_u \geq budget$} {															\label{h-mcts:shot}
		...
		$s \gets \ceil{\nicefrac{s}{2}}$, $k \gets k + 1$							\;
		$S_{k} \gets$ the $s$ arms from $S_{k-1}$ with the best average				\;	\label{h-mcts:sort}
...
In the form presented in Algorithm~\ref{alg:h-mcts}, H-MCTS cannot solve proven wins or losses in the simple regret tree. Although we can employ the MCTS-Solver proposed by~\citeay{Winands2008} in the UCT tree, this solver needs to be adapted to be able to solve nodes in the SHOT portion of the algorithm. The main issue here is that when a solved node is encountered, Sequential Halving must be interrupted. This leads to three different possibles cases in which this interruption is to be encountered and properly handled. Note that it is assumed that values are according to negamax, such that at each node, children's rewards are stored with respect to the player to move.
\begin{enumerate}
	...
	\Repeat{$b_u \geq budget$} {
		...
		\For{i=0 \emph{\KwTo} $\min{(s,|S_k|)}$}{
			...
			\uIf{$n_i$ is a win \textbf{or} $(n_i$ is a loss \textbf{and} $\forall n \in S: (n$ is a loss$))$}{
				...
				$i \gets i - 1$													\; \label{hs:reselect}
			}
			break if $b_u \geq budget$											\;
		}
		$s \gets \ceil{\nicefrac{s}{2}}$, $k \gets k + 1$						\;
		$s \gets \min{(s, |S|)}$												\; \label{hs:size_min}	
		$S_{k} \gets$ the $s$ arms from $S_{k-1}$ with the best average			\;		
		$b \gets b + \max{\left(1, \floor[\bigg]{\frac{p.budgetSpent + budget}{s\times\ceil{log_2{|S|}}}}\right)}$ \;
