...
\item{\emph{Will using simple regret minimizing, pure exploration selection policies in MCTS improve performance in two-player games?}}
\newline
\NoIndent{This question is answered by performing experiments in six distinct two-player games: Amazons, AtariGo, Ataxx, Breakthrough, NoGo, and Pentalath. The last research question relates to an existing MCTS enhancement:}

\item{\emph{How can the MCTS-Solver by~\citeay{Winands2008}, be adapted to work with the new MCTS technique?}}
\newline
\NoIndent{This will be answered by adapting and implementing the solver in the new algorithm, and by experimentation, to determine whether it improves results.}
\end{enumerate}
...
Section \ref{sec:hybmcts} gives an introduction and analysis of H-MCTS. Next, Section \ref{sec:hybmctssolver} details the implementation of MCTS-Solver~\citeaby{Winands2008}, designed to solve positions within the tree, for H-MCTS. Finally, in Section \ref{sec:ulb_h-mcts}, an enhancement for SHOT and H-MCTS is introduced that reduces the number of poor nodes selected based on their upper bounds overlapping with their best sibling's lower bound.
\newpage
\section{Hybrid MCTS}
\label{sec:hybmcts}
...
\setstretch{1.1}
	...
	\Repeat{$b_u \geq budget$} {															\label{h-mcts:shot}
		...
		$s \gets s - \floor{\nicefrac{s}{2}}$, $k \gets k + 1$							\;
		$S_{k} \gets$ $S_{k-1}$ sorted in descending order							\;	\label{h-mcts:sort}
...
Some changes are made in H-MCTS with respect to SHOT. The cycle at line~\ref{h-mcts:shot} repeats until no more budget remains instead of repeating until a single child remains. Because it is possible, due to rounding errors, that not all allocated budget is spent during a cycle, it is not unlikely to have `residual' rounds, in which the single best child is the only one remaining. Sampling this child more frequently is never a problem, since at each node we are most interested in finding the best reply, and updating the parent's value accordingly.
...
When all $T$ simulations have been performed, H-MCTS recommends the single remaining node in $n_0 \in S$ at the root. Note that, this is not necessarily the one with the highest value, it is possible that the best node's value has decreased after several rounds. However, we can not assume that the values of the previously removed nodes will not also decrease given more samples. Therefore, because most samples were spent on $n_0$, we are the most confident in its value. Other nodes with less visits may have higher values, but the confidence in these values is much lower. A possible solution to this problem presents itself in the manner that $S_k$ is constructed, on line~\ref{h-mcts:sort}. An alternative would be to setup each round's subset of nodes as follows: $S_k \gets S$ sorted in descending order. This way, nodes that were previously removed from selection can `return', and replace nodes who's value decreased.
...
In the form presented in Algorithm~\ref{alg:h-mcts}, H-MCTS cannot solve proven wins or losses in the simple regret tree. Although we can employ the MCTS-Solver proposed by~\citeay{Winands2008} in the UCT tree, this solver is to be adapted to be able to solve nodes in the SHOT portion of the algorithm.
\newpage
\section{Hybrid MCTS-Solver}
\label{sec:hybmctssolver}

In the form presented in Algorithm~\ref{alg:h-mcts}, H-MCTS cannot solve proven wins or losses in the simple regret tree. Although we can employ the MCTS-Solver proposed by~\citeay{Winands2008} in the UCT tree, the technique requires adaptation to Sequential Halving to be able to solve nodes in the SHOT portion of the algorithm. The main issue here is that when a solved child is encountered, the current round is interrupted to determine whether the node itself is solved as well. This leads to three different possibles cases in which this interruption is to be encountered and properly handled. Note that it is assumed that values are according to negamax, such that at each node, children's rewards are stored with respect to the player to move.
\begin{enumerate}
...
\setstretch{1.0}
	...
	\Indp
	\lIf{isLeaf($p$)}{$C, S\gets$ \func{expand}($p$)}								\label{hs:children}
	$t_s \gets \tuple{0,0,0,0}$														\;		
	...
	\Repeat{$b_u \geq budget$} {
		...
		\For{i=0 \emph{\KwTo} $\min{(s,|S_k|)}$}{
			...
			\uIf{$n_i$ is a win \textbf{or} $(n_i$ is a loss \textbf{and} $\forall n \in C: (n$ is a loss$))$}{
				set the solved player in $t_s$									\; \label{hs:solved}
				\func{update} $p.budgetSpent$ with $b_u$						\;
				\KwRet{$t_s$}													\;
			} \ElseIf{$(n_i$ is a loss \textbf{and} $\exists n \in C: (n$ is not a loss$))$}{
				remove $n_i$ from $S$											\; \label{hs:remove}			
		...
		$S_{k} \gets$ $S_{k-1}$ sorted in descending order						\; \label{hs:sort}	
		\lIf{$s = 1$} {$b \gets b + (budget - b_u)$}
		\Else {	
			$b \gets b + \max{\left(1, \floor[\bigg]{\frac{p.budgetSpent + budget}{s\times\ceil{log_2{|S|}}}}\right)}$ \;
...
\setstretch{0.9}
	...
	\func{h-mcts-solver}(node $p$, $budget$):										\;
	\Indp
	\lIf{isLeaf($p$)}{$C, S\gets$ \func{expand}($p$)}					
	$t_s \gets \tuple{0,0,0,0}$														\;		
	$b \gets \max{\left(1, \floor[\bigg]{\frac{p.budgetSpent + budget}{s\times\ceil{log_2{|S|}}}}\right)}$ \; 
	\If{$p$ is not root \textbf{and} $b < B $}{
		\For{i=0 \emph{\KwTo} budget}{
		...
		\For{i=0 \emph{\KwTo} $\min{(s,|S_k|)}$}{
			...
			\If{$n_i$ is not solved} {			
				\If{$b > n_i.visits$} {
					...
					\If {$n_i.value + C \times \sqrt{\frac{\ln{p.visits}}{n_i.visits}} < l_b$} { \label{ublb:upper_bound}
						$b_r \gets b_r + b_i$ \;									
