...
\newline
\NoIndent{This question is answered by performing experiments in four different two-player games: Amazons, Breakthrough, Chinese Checkers and Pentalath. The last research question relates to an existing MCTS enhancement:}
...
\setstretch{1.1}
	...
	\If{isTerminal($p$)}{															\label{h-mcts:terminal}
		\func{update} $t_s$, with $budget$ wins 									\;
		 \KwRet{$t_s$}
	}
	...
	\Repeat{$b_u \geq budget$} {
		...
		\For{i=0 \emph{\KwTo} $\min{(s,|S_k|)}$}{
			...
			\uIf{$n_i$ is a win \textbf{or} $(n_i$ is a loss \textbf{and} $\forall n \in C: (n$ is a loss$))$}{
				...
				$b_r \gets b_i - t_s\tuple{v}$									\; \label{hs:update_resid}				
			}
			break if $b_u \geq budget$											\;
		}
		$s \gets \ceil{\nicefrac{s}{2}}$, $k \gets k + 1$						\;
