...
\begin{algorithm2e}[ht]
	...
	\For{k=1 \emph{\KwTo} $|K|-1$}{
		Select each arm $i\in S_k$ $n_k - n_{k-1}$ times 						\;	\label{succrej:subtr}
...
\setstretch{.92}
	...
	\If{isTerminal($p$)}{																	\label{shot:terminal}
		\func{update} $t_s$, with 1 win and 1 visit										\;
		\KwRet{$t_s$}																	\;
	}
	...
	\Indm
	\KwRet{$t_s$}
  \caption{Sequential Halving applied to Trees (SHOT)~\protect\citebay{Cazenave14SHOT}. \label{alg:shot}}
\end{algorithm2e}
\DecMargin{1em}
