\documentclass{icga}
\usepackage{graphicx, epstopdf, amsmath, mathtools, amssymb, color, subcaption, comment, booktabs, mathabx,  varwidth, setspace, algorithm, fixltx2e, nicefrac, pgfplots, tikz, etoolbox, etex}
...
\setstretch{1.1}
	...
	$b_r \gets 0$ \; \label{ss:resid}
		...
		$s \gets \ceil{\nicefrac{s}{2}}$													\;
		$s \gets \min{(s, |S|)}$												\; \label{ss:size_min}	
		\lIf{$s = 1$} {$b \gets b + (budget - b_u)$}
		\Else {	
			$b \gets b + \max{\left(1, \floor[\bigg]{\frac{p.budgetSpent + budget}{s\times\ceil{log_2{|S|}}}}\right)}$ \;
			$b \gets b + \ceil{\nicefrac{b_r}{s}}$														\; \label{ss:incr_budget_resid}
	}
	\func{update} $p.budgetSpent$ with $b_u$												\;
	\Indm
	\KwRet{$t_p$}
  \caption[SHOT Solver]{SHOT Solver. \label{alg:shot-solver}}
\end{algorithm2e}
\DecMargin{1em}
