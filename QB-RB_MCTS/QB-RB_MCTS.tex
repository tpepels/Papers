...
Determining the qualitative bonus follows the same procedure as the relative bonus. Similar to RB a distribution over observed $q$ values is maintained $Q \sim \mathcal{N}(\bar{Q}, \hat{\sigma}_q)$ for each player $w$. Furthermore, the value of $q$ is determined by a domain dependent assessment of quality of the terminal state. Assuming that higher values of $q$ represent a higher score for the winning player $w$, then $\delta_q$ is defined as:
\begin{equation}
\delta_q = q - \bar{Q}^w
\end{equation}
Consequently, $\lambda$ becomes:
\begin{equation}
